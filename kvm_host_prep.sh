...
sudo apt -y -q install ufw fail2ban htop net-tools qemu-kvm libvirt-bin qemu-utils genisoimage virtinst cloud-image-utils

echo "Increase file limits"
LIMITS=$(cat << 'EOF'
*           hard   nofile   1048576
*           soft   nofile   1048576
tifintech   hard   nofile   1048576
tifintech   soft   nofile   1048576
EOF
)
grep -qxF "$LIMITS" /etc/security/limits.conf || echo "$LIMITS" | sudo tee -a /etc/security/limits.conf
